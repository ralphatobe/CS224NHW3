...
def make_windowed_data(data, start, end, window_size = 1):
    ...
    ### YOUR CODE HERE (5-20 lines)
        ...
        for i in range(k):
            ...
            # add starts
            if i - window_size < 0:
                features = start * (window_size - i) + features
                # print features
            # add features
            features = features + sum(sentence[max(0,i - window_size):min(k,i + window_size + 1)], [])
            # print features
            # add ends
            if i + window_size + 1 > k:
                features = features + end * (i + window_size + 1 - k)
                # print features
            # add to output list
...
class WindowModel(NERModel):
    ...
    def add_embedding(self):
        ...
        embeddings = tf.reshape(embeddings2,[-1,self.config.n_window_features*self.config.embed_size])                                                                               
        ### END YOUR CODE
        return embeddings
